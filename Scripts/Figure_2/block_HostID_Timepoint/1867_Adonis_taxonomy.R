...
sample_metadata_taxonomy_file <- "../../../data/1867/taxonomy/1867_taxonomy_metadata.tsv" 

dm_taxo_phylo_rpca_file <- "../../../data/1867/taxonomy/1867_all_asv_phylo_rpca/phylo_rpca.distance_matrix/distance-matrix.tsv"

outpath <- "../../../Results/Figure_2/1867_taxonomy_dist_block_hostid_timepoint.PerMANOVA_out/"
dir.create(outpath)

#-------------------------------
# Sample Metadata input
#-------------------------------
sample_metadata_taxonomy <- read.table(sample_metadata_taxonomy_file,header=T,sep="\t",row.names=1, quote="", comment.char="")
...
md_sample_metadata_taxonomy_summ<-check_metadata(metadata = sample_metadata_taxonomy)
write.table(md_sample_metadata_taxonomy_summ, paste0(outpath, "md_sample_metadata_taxonomy_summ.tsv"), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA)


# remove two columns on sampling date 
idx = !grepl("date", md_sample_metadata_taxonomy_summ$metadata)
md_sample_metadata_taxonomy_summ <- md_sample_metadata_taxonomy_summ[idx, ]

file_list <- list(dm_taxo_phylo_rpca_file)
...
dm_list<-lapply(dm_list, function(x){filter_samples_by_sample_ids_in_metadata(x, dm=TRUE, sample_metadata_taxonomy)})
