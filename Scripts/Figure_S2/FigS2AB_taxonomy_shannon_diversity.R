...
shannon_diversity_position_group_boxplot <- function(abd_table, metadata, group, mapping = data.frame(), outpath = ".") {
  ...
  write.table(diversity_index, paste0(outpath, "/taxonomy_", group, "_diversity.txt"), sep = "\t", quote = F, row.names = T, col.names = F)
  ...
  ggsave(filename=paste0(outpath, "/taxonomy_", group, "_position_HostGroup.pdf"), plot=plot, width=8, height=3)
}

metadata <- read.table("../../data/637/taxonomy/637_taxonomy_metadata.tsv")
...
metadata <- read.table("../../data/1867/taxonomy/1867_taxonomy_metadata.tsv", sep = "\t", header = T, row.names = 1)
...
abd_table <- read_hdf5_biom("../../data/637/taxonomy/silva_taxonomy_collapse/level8/feature-table.biom")
abd_table <- biom(abd_table)
abd_table <- data.frame(t(as.matrix(biom_data(abd_table))), check.names = FALSE)
abd_table <- abd_table[order(rownames(abd_table)), ]
metadata <- read.table("../../data/637/taxonomy/637_taxonomy_metadata.tsv", sep = "\t", header = T, row.names = 1)
identical(rownames(abd_table), rownames(metadata))
