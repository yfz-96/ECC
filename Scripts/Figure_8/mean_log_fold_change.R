...
p.adj.method="BH"
h=10
outpath <- paste("../../Results/Figure_8/mean_log2_fold_change/", sep="")
...
mean_logfc <- Mean_logfc(HHCC_df, factor(HHCC_metadata$Future_Status_Tooth), positive_class = "Caries")
mean_logfc <- data.frame(mean_logfc)
write.table(mean_logfc, paste0(outpath, "/HHCC_mean_log_fold_change.txt"), sep = "\t", quote = F, row.names = T, col.names = NA)
...
# status_related_features_mean_logfc <- subset(mean_logfc, rownames(mean_logfc) %in% status_related_features$feature)
status_related_features_mean_logfc <- mean_logfc[which(rownames(mean_logfc) %in% status_related_features$feature), ]
...
mean_logfc.by_datasets <- function(df, metadata, s_category, c_category, positive_class) {
  ...
  oper<-foreach(i=1:L, .combine='cbind', .multicombine=TRUE) %dopar% {
    ...
    y<-factor(y_list[[i]])
    # 2. mean fold change
    oob <- Mean_logfc(x, y,  positive_class = positive_class)
    oob
  }
...
mean_logfc <- mean_logfc.by_datasets(data_HHCC_list$data, data_HHCC_list$metadata, 
                               s_category, c_category, positive_class = "Caries")
write.table(mean_logfc, paste0(outpath, "/mean_logfc_by_datasets.txt"),
...
rownames(status_related_features_mean_logfc) <- Taxonomy[rownames(status_related_features_mean_logfc), "Taxon"]

status_related_features_mean_logfc <- melt(status_related_features_mean_logfc)
status_related_features_mean_logfc$Var2 <- factor(status_related_features_mean_logfc$Var2,
                                               levels = c("T55", "T54", "T5161", "T64", "T65", "T75", "T74", "T84", "T85"),
                                               ordered = T)
plot <- ggplot(status_related_features_mean_logfc, aes(Var2, Var1, fill = value)) +
  geom_tile() +
...
plot
ggsave(filename=paste0(outpath, "/mean_log2fold_change_of_status_related_asvs_for_ECC_diagnosis.pdf"), plot=plot, width=8.5, height=3.5)
