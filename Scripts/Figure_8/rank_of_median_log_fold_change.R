...
median_logfc <- read.table(paste0(outpath, "/median_logfc_by_datasets.txt"),
            sep = "\t", header = T, row.names = 1)
median_logfc <- apply(median_logfc, 2, function(x) {rank(-x)})
...
plot
ggsave(filename=paste0(outpath, "/rank_of_median_log2fold_change_of_status_related_asvs_for_ECC_diagnosis.pdf"), plot=plot, width=8.5, height=3.5)
