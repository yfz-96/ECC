...
p.adj.method="BH"
h=10
outpath <- "../../Results/Figure_8/mean_log2_fold_change/"
dir.create(outpath, recursive = T)

mean_logfc <- read.table(paste0(outpath, "/mean_logfc_by_datasets.txt"),
            sep = "\t", header = T, row.names = 1)
mean_logfc <- apply(mean_logfc, 2, function(x) {rank(-x)})
...
plot
ggsave(filename=paste0(outpath, "/rank_of_mean_log2fold_change_of_status_related_asvs_for_ECC_diagnosis.pdf"), plot=plot, width=8.5, height=3.5)
