...
p.adj.method="BH"
h=10
outpath <- paste("../../Results/Figure_8/generalized_log2_fold_change_of_kos/", sep="")
...
status_related_features <- read.table("../../Results/Figure_6/MiC/function_Position_crossRF_out/BetweenGroupTest_out_all.xls", 
                                      sep = "\t", header = T)
...
status_related_features_glogfc <- glogfc[which(rownames(glogfc) %in% status_related_features$feature), ]
...
g_logfc <- g_logfc.by_datasets(data_HHCC_list$data, data_HHCC_list$metadata, 
                               s_category, c_category, positive_class = "Caries")
write.table(g_logfc, paste0(outpath, "/generalized_logfc_by_datasets.txt"),
...
Taxonomy <- read.table("../../data/1867/function/KO/1867_function_taxonomy.txt", 
                       sep = "\t", header = T, row.names = 1, comment.char = "")
Taxonomy$Taxon <- with(Taxonomy, paste(Taxon, rownames(Taxonomy), sep = "_"))
