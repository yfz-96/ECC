...
taxonomy_dist <- read.table("../../data/1867/taxonomy/1867_all_asv_phylo_rpca/phylo_rpca.distance_matrix/distance-matrix.tsv",
...
write.table(mic_dist_features, paste0(outdir, "/1867_all_taxonomy_dist_features.txt"), sep = "\t", quote = F, row.names = T, col.names = NA)
...
p
ggsave(filename= paste0(outdir, "/Fig5B_1867_all_taxonomy_dist_features1.pdf"),plot=p, width=7, height=3)
...
p <- ggplot(df2, aes(x = Future_Status_Tooth, y = value)) + 
  ...
  theme(strip.text = element_text(size = 14),
        ...
        panel.background = element_blank())
p
ggsave(filename= paste0(outdir, "/Fig5D_1867_all_taxonomy_dist_features2.pdf"),plot=p, width=7, height=7)
