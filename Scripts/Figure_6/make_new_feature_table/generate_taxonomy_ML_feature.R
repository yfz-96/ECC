...
mic_abd_637 <- read_hdf5_biom("../../../data/637/taxonomy/silva_taxonomy_collapse/level8/feature-table.biom")
mic_abd_637 <- biom(mic_abd_637)
mic_abd_637 <- as.matrix(biom_data(mic_abd_637))
col_sums <- colSums(mic_abd_637)
mic_abd_637 <- sweep(mic_abd_637, 2, col_sums, FUN="/")
mic_abd_637 <- mic_abd_637[, order(colnames(mic_abd_637))]
clinical_features_637 <- read.table("../../../Results/Figure_S5/637_clinical_features.txt",
                                    sep = "\t", header = T, row.names = 1)
clinical_features_637 <- as.matrix(clinical_features_637)
clinical_features_637 <- t(clinical_features_637)
dist_features_637 <- read.table("../../../Results/Figure_S5/637_all_taxonomy_dist_features.txt", 
                                sep = "\t", header = T, row.names = 1)
...
taxo_mic_637 <- read.table("../../../data/637/taxonomy/silva_taxonomy_collapse/level8/taxonomy.tsv", 
                           sep = "\t", header = T, row.names = 1, quote = "", comment.char = "")
taxo_mic_637 <- taxo_mic_637[, c("Taxon", "Confidence")]
taxo_new_features_637 <- read.table("taxo_new_taxonomy_feature.txt", sep = "\t", header = T, row.names = 1)
...
write_biom(all_features_table_biom_637, paste0(outdir, "/637_taxonomy_feature_table.biom"))
write.table(taxo_all_features_637, paste0(outdir, "/637_taxonomy_feature_taxon.txt"), 
            sep = "\t", quote = F, row.names = T, col.names = NA)



mic_abd_1867 <- read_hdf5_biom("../../../data/1867/taxonomy/silva_taxonomy_collapse/level8/feature-table.biom")
...
dist_features_1867 <- read.table("../../../Results/Figure_5/1867_all_taxonomy_dist_features.txt", 
                                sep = "\t", header = T, row.names = 1)
...
taxo_mic_1867 <- read.table("../../../data/1867/taxonomy/silva_taxonomy_collapse/level8/taxonomy.tsv", 
                           sep = "\t", header = T, row.names = 1, quote = "", comment.char = "")
taxo_mic_1867 <- taxo_mic_1867[, c("Taxon", "Confidence")]
taxo_new_features_1867 <- read.table("taxo_new_taxonomy_feature.txt", sep = "\t", header = T, row.names = 1)
...
write_biom(all_features_table_biom_1867, paste0(outdir, "/1867_taxonomy_feature_table.biom"))
write.table(taxo_all_features_1867, paste0(outdir, "/1867_taxonomy_feature_taxon.txt"), 
            sep = "\t", quote = F, row.names = T, col.names = NA)
