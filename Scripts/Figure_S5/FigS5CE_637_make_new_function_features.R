...
function_dist <- read.table("../../data/637/function/637_all_ko_rpca/rpca.distance_matrix/distance-matrix.tsv",
...
for(sample in all_samples) {
  ...
  if(length(other_samples) > 0) {
    mic_dist_features[sample, "mean_md"] <- mean(unlist(function_dist[sample, other_samples]))
    mic_dist_features[sample, "spatial_dist_weighted_mean_md"] <-
      mean(dist[sample, other_samples] * unlist(function_dist[sample, other_samples])) 
  }
}

library(stringr)
function_dist <- subset(function_dist, metadata[rownames(function_dist), "HostGroup"] == "H2H")
mean_all_dist <- apply(function_dist, 2, function(x) {aggregate(x, by=list(type=metadata[rownames(function_dist), "Position2"]), mean)})
...
colnames(mean_all_dist) <- str_sub(colnames(mean_all_dist), 1, nchar(colnames(mean_all_dist))-2)
...
write.table(mic_dist_features, paste0(outdir, "/637_all_function_dist_features.txt"), sep = "\t", quote = F, row.names = T, col.names = NA)
...
p
ggsave(filename= paste0(outdir, "/FigS5C_637_all_function_dist_features1.pdf"),plot=p, width=7, height=3)
...
p <- ggplot(df2, aes(x = Future_Status_Tooth, y = value)) + 
  ...
  theme(strip.text = element_text(size = 14),
        ...
        panel.background = element_blank())
p
ggsave(filename= paste0(outdir, "/FigS5E_637_all_function_dist_features2.pdf"),plot=p, width=10, height=10)
